...
%sam
\subsection{EM observables and parameters set}
%sam
For a catalogue candidate c, the available EM information may be represented schematically as
\begin{equation}
d_{EM,c} = {\Omega_{l,c},z_{l,c},F_{\nu,c},q_c,R_{eff,c},\sigma_{v,c},...}
\end{equation}
The specific set is catalogue dependent.
Spectroscopic redshifts can be represented by a narrow likelihood; photometric redshifts should retain the full probability distributions.
For BBH systems without an identified source counterpart, $z_s$ is latent and must be marginalised using the GW distance information, cosmology, magnification model and source population prior.\\
\\The $H_2$ parameter vector is
\begin{equation}
\Theta_2=\{\theta_{BBH},\theta_{lens},\beta,\nu_{EM}\}
\end{equation}
Conditional on $\Theta_2$, the detector strain data and galaxy observations are taken to be independent measurements of the common physical system,
\begin{equation}
p(d_{GW},d_{EM,c}|\Theta_2,H_2) = p(d_{GW}|\theta_{BBH},\theta_{lens},\beta,H_2)p(d_{EM,c}|\theta_{lens},\nu_{EM},H_2)
\end{equation}
